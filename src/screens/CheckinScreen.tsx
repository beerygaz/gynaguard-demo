...
const flowOptions: { value: FlowLevel; label: string; dot: string }[] = [
  { value: 'none', label: 'None', dot: 'bg-warm-muted' },
  { value: 'spotting', label: 'Spotting', dot: 'bg-rust-300' },
  { value: 'light', label: 'Light', dot: 'bg-rust-400' },
  { value: 'medium', label: 'Medium', dot: 'bg-rust-500' },
  { value: 'heavy', label: 'Heavy', dot: 'bg-rust-600' },
  { value: 'very_heavy', label: 'Very heavy', dot: 'bg-rust-800' },
]
  ...
  if (step === 'done') {
    return (
      <div className="max-w-lg mx-auto px-4 pt-20 pb-8 flex flex-col items-center text-center gap-6">
        <div className="w-20 h-20 gradient-rust rounded-full flex items-center justify-center shadow-xl">
          <Check size={36} className="text-white" />
        </div>
        <div>
          <h2 className="text-display text-2xl font-semibold text-warm-text">All logged!</h2>
          <p className="text-warm-muted mt-2">Day {currentCycleDay} captured. See you tomorrow.</p>
        </div>
        <div className="w-full bg-warm-card border border-warm-border rounded-2xl p-4 text-left space-y-2">
          <p className="text-xs text-warm-muted uppercase tracking-wider font-medium mb-3">Today's summary</p>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-10 pb-4">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={back} className="w-9 h-9 rounded-full bg-warm-card border border-warm-border flex items-center justify-center">
          <ChevronLeft size={18} className="text-warm-muted" />
        </button>
        <div className="flex-1">
          <p className="text-warm-muted text-xs">Daily check-in · Day {currentCycleDay}</p>
          <div className="flex gap-1 mt-1.5">
            {steps.slice(0, -1).map((s, i) => (
              <div
                key={s}
                className={`h-1 flex-1 rounded-full transition-all ${
                  i <= stepIdx ? 'bg-rust-500' : 'bg-warm-border'
                }`}
              />
            ))}
          </div>
          ...
          <div className="space-y-2">
            {flowOptions.map(o => (
              ...
              >
                ...
                <span className={`font-medium ${flow === o.value ? 'text-rust-300' : 'text-warm-text'}`}>
                  {o.label}
                </span>
                {flow === o.value && (
                  <Check size={14} className="text-rust-400 ml-auto" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {moodOptions.map(m => (
              ...
              >
                ...
                <span className={`text-xs font-medium ${moods.includes(m.value) ? 'text-rust-300' : 'text-warm-muted'}`}>
                  {m.label}
                </span>
              </button>
            ))}
      ...
      {step === 'symptoms' && (
        <StepWrapper title="Any symptoms?" subtitle="Select all that apply — or skip if none" onNext={next}>
          ...
          <div className="mt-5 border-t border-warm-border pt-5">
            <p className="text-warm-muted text-xs uppercase tracking-wider font-medium mb-3">Also log</p>
            <button
              onClick={() => setIntimacy(v => !v)}
              className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl border transition-all ${
                intimacy ? 'border-rust-500 bg-rust-900/30' : 'border-warm-border bg-warm-card'
              }`}
            >
              <span className="text-base">🤍</span>
              <span className={`font-medium text-sm ${intimacy ? 'text-rust-300' : 'text-warm-text'}`}>Intimacy</span>
              {intimacy && <Check size={14} className="text-rust-400 ml-auto" />}
            </button>
          </div>
        </StepWrapper>
      )}
      ...
      {step === 'notes' && (
        <StepWrapper title="Anything else?" subtitle="Optional — a private note for today" onNext={next} nextLabel="Save log">
          ...
          />
          <p className="text-warm-muted text-xs mt-2">Only you can see this.</p>
        </StepWrapper>
      )}
    </div>
  )
...
function StepWrapper({
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-display text-2xl font-semibold text-warm-text">{title}</h2>
        {subtitle && <p className="text-warm-muted text-sm mt-1">{subtitle}</p>}
      </div>
      {children}
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between text-sm">
      <span className="text-warm-muted capitalize">{label}</span>
      <span className="text-warm-text text-right max-w-[60%] capitalize">{value}</span>
    </div>
  )
}
